...
for record in tree.xpath('.//record'): # Selects all subelements, on all levels beneath the current element. For example, .//egg selects all egg elements in the entire tree.
    ...
    if status is None:
        ...
        elements=record.findall(paths['dates'], namespaces=ns)
        ...
        elements=record.findall(paths['creators'], namespaces=ns)
        print_elements(elements)

        elements=record.findall(paths['contributors'], namespaces=ns)
        print_elements(elements)

        elements=record.findall(paths['subjects'], namespaces=ns)
        print_elements(elements)
